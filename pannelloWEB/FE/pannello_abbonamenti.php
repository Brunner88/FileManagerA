...
<body>
  ...
  <h2 class="section-title">Totali Abbonati</h2>
  <div id="chartsMain" class="charts-grid"></div>

  <h2 class="section-title">Provenienze Abbonati</h2>
  ...
  <script>
    ...
    async function getDataSub(period="yesterday") {
      const res = await fetch(`../BE/get_data_sub.php?period=${period}`);
      return await res.json();
    }

    async function getDataSubFrom(period="yesterday") {
      const res = await fetch(`../BE/get_data_sub_from.php?period=${period}`);
      return await res.json();
    }
