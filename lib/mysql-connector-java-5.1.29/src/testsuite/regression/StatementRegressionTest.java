...
public class StatementRegressionTest extends BaseTestCase {
	...
	public void testBug11540() throws Exception {
		...
		try {
			...
			thaiPrepStmt = ((Connection) thaiConn)
	...
	public void testBug15141() throws Exception {
		try {
			...
			this.pstmt = ((Connection) this.conn)
	...
	public void testBug3557() throws Exception {
		boolean populateDefaults = ((ConnectionProperties) this.conn)
	...
	public void testBug4718() throws SQLException {
		if (versionMeetsMinimum(4, 1, 0)
			...
			assertTrue(this.pstmt instanceof PreparedStatement);
		...
		try {
			...
			int updateCount = this.stmt
					.executeUpdate("LOAD DATA LOCAL INFILE '"
							...
							" CHARACTER SET " + CharsetMapping.getMysqlEncodingForJavaEncoding(((MySQLConnection)this.conn).getEncoding(), (Connection) this.conn));
	...
	public void testSetCharacterStream() throws Exception {
		try {
			((Connection) this.conn).setTraceProtocol(true);
			...
			((Connection) this.conn).setTraceProtocol(false);
	...
	public void testBug20029() throws Exception {
		...
		for (int i = 0; i < 10; i++) {
			...
			try {
				toBeKilledPstmt = ((Connection) toBeKilledConn)
	...
	public void testBug22290() throws Exception {
		...
		try {
			...
			assertEquals(
					...
					1);
			this.pstmt = ((Connection) configuredConn)
	...
	public void testBug25073() throws Exception {
		...
		int beforeOpenStatementCount = ((Connection) multiConn)
				.getActiveStatementCount();

		multiStmt.executeBatch();

		int afterOpenStatementCount = ((Connection) multiConn)
				.getActiveStatementCount();
		...
		beforeOpenStatementCount = ((Connection) multiConn)
				.getActiveStatementCount();

		multiStmt.executeBatch();

		afterOpenStatementCount = ((Connection) multiConn)
				.getActiveStatementCount();
	...
	public void testBug28256() throws Exception {
		try {
			...
			if (versionMeetsMinimum(4, 1)) {
				((Connection) this.conn).serverPrepareStatement(
	...
	public void testBug34518() throws Exception {
		...
		int stmtCount = ((Connection) fetchConn)
				.getActiveStatementCount();
	...
	public void testBug35666() throws Exception {
		...
		this.pstmt = ((Connection) loggingConn)
	...
	private void checkOpenResultsFor44056(Statement newStmt)
		...
		assertEquals(0,
				((Statement) newStmt).getOpenResultSetCount());
		this.rs.close();
		assertEquals(0,
				((Statement) newStmt).getOpenResultSetCount());
	}
	...
	public static class Bug39426Interceptor implements StatementInterceptor {
		...
		public ResultSetInternalMethods preProcess(String sql,
			...
			if (interceptedStatement instanceof PreparedStatement) {
				...
				prevSql = asSql;
				ParameterBindings b = ((PreparedStatement) interceptedStatement)
	...
	public void testBug61501() throws Exception {
		...
		stmt.cancel();
		stmt.execute(sql);
		pstmt = ((Connection) conn).serverPrepareStatement(sql);
		pstmt.execute();
		pstmt.cancel();
		pstmt.execute();
