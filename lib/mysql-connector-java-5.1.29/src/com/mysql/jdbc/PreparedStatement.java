 ...
 */
public class PreparedStatement extends StatementImpl implements
	...
	public PreparedStatement(MySQLConnection conn, String sql, String catalog)
		...
		if (this.originalSql.startsWith(StatementImpl.PING_MARKER)) {
			...
			try {
				try {
					...
					if (locallyScopedConn.getEnableQueryTimeouts()
							&& batchTimeout != 0
							&& locallyScopedConn.versionMeetsMinimum(5, 0, 0)) {
						timeoutTask = new StatementImpl.CancelTask(
		...
		synchronized (checkClosed().getConnectionMutex()) {
			...
			if (doStreaming
				...
				try {
					...
					((StatementImpl)stmt).executeSimpleNonQuery(this.connection, "SET net_write_timeout="
